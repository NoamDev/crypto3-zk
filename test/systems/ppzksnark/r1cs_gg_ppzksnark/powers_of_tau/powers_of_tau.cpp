...
BOOST_AUTO_TEST_CASE(powers_of_tau_result_basic_test) {
    ...
    for(std::size_t m = tau_powers; m > tau_powers/2; --m) {
        auto domain = nil::crypto3::math::make_basic_evaluation_domain<scalar_field_type>(m);
        auto u = domain->evaluate_all_lagrange_polynomials(sk.tau);

        BOOST_CHECK(u.size() == domain->m);

        for(std::size_t i = 0; i < domain->m; ++i) {
            BOOST_CHECK_MESSAGE(result.coeffs_g1[i] == g1_generator * u[i], std::string("i=") + std::to_string(i) + std::string("m=") + std::to_string(m));
            BOOST_CHECK_MESSAGE(result.coeffs_g2[i] == g2_generator * u[i], std::string("i=") + std::to_string(i) + std::string("m=") + std::to_string(m));
            BOOST_CHECK_MESSAGE(result.alpha_coeffs_g1[i] == g1_generator *(sk.alpha * u[i]), std::string("i=") + std::to_string(i) + std::string("m=") + std::to_string(m));
            BOOST_CHECK_MESSAGE(result.beta_coeffs_g1[i] == g1_generator * (sk.beta * u[i]), std::string("i=") + std::to_string(i) + std::string("m=") + std::to_string(m));
        }
    }
}
